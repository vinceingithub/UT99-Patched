
╭╮╱╱╭╮╱╱╱╱╱╱╱╱╱╱╱╱╱╭━━━╮╱╱╱╱╱╱╱╱╱╱╱╱╱╭╮╭╮╱╭┳━━━━┳━━━┳━━━╮
┃╰╮╭╯┃╱╱╱╱╱╱╱╱╭╮╱╱╱┃╭━╮┃╱╱╱╱╱╱╱╱╱╱╱╱╱┃┃┃┃╱┃┃╭╮╭╮┃╭━╮┃╭━╮┃
╰╮┃┃╭╋━╮╭━━┳━━┫┣━━╮┃┃╱╰╋━━┳━╮╭━╮╭━━┳━╯┃┃┃╱┃┣╯┃┃╰┫╰━╯┃╰━╯┃
╱┃╰╯┣┫╭╮┫╭━┫┃━╋┫━━┫┃┃╱╭┫╭╮┃╭╮┫╭╮┫┃━┫╭╮┃┃┃╱┃┃╱┃┃╱╰━━╮┣━━╮┃
╱╰╮╭┫┃┃┃┃╰━┫┃━┫┣━━┃┃╰━╯┃╭╮┃┃┃┃┃┃┃┃━┫╰╯┃┃╰━╯┃╱┃┃╱╭━━╯┣━━╯┃
╱╱╰╯╰┻╯╰┻━━┻━━╯╰━━╯╰━━━┻╯╰┻╯╰┻╯╰┻━━┻━━╯╰━━━╯╱╰╯╱╰━━━┻━━━╯

Thank you for choosing Vince's Canned Unreal Tournament!

This is a pre-patched version of the Steam release Unreal Tournament 1999 for Agora Road frag nights.

CHANGES:
+ Removed bloat items
+ Changed video renderer to DirectX9
+ Changed movement controls to standard WASD
+ Added and configured the ChaosUT mod
+ Added and configured the UltraGoreSSE mod
+ Cached all maps and textures used on the Agora Road's Unreal Tournament server
+ Patched game with years worth of various bug fixes and performance enhancements

HOW DO I PLAY?:
1. Double click the "Enter the Tournament" short cut
2. Press the Esc key to skip the intro cutscene
3. Click "Options" at the top to open the drop down
4. Click "Player Setup" and change your character model, username, and more (click "Close" when done)
5. Click "Multiplayer" at the top to open the drop down
6. Click "Open Location" and type the server IP "vince.webhop.me"
7. When prompted for the password, type "Ag0raCHANis_myW1F3"
