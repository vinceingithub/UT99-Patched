...
static const float3 LUMINANCE_VECTOR = float3(0.2f, 0.5f, 0.1f);
static const float3 BLUE_SHIFT_VECTOR = float3(1.05f, 0.97f, 1.27f); 
static const float  MIDDLE_GRAY = 0.1f;//0.07f
...
{
	uniform float hdrLuminance;
	uniform float hdrAdaptation;
...
PS_OUTPUT_SIMPLE_R32 PS_sourceToLum(PS_INPUT_SIMPLE input)
{
    ...
    [unroll] for( int y = -1; y < 2; y++ )//def 2x2
    {
...
PS_OUTPUT_SIMPLE_R32 PS_calculateAdaptedLum(PS_INPUT_SIMPLE input)
{
	...
	float fNewAdaptation = fAdaptedLum + (fCurrentLum - fAdaptedLum) * ( 1 - pow( abs(0.9+hdrAdaptation), 60 * elapsedTime ));
	if(fNewAdaptation>0.1f)
		fNewAdaptation=0.1f;
...
PS_OUTPUT_SIMPLE PS_brightPass(PS_INPUT_SIMPLE input)
{
    ...
    vColor *= (MIDDLE_GRAY+hdrLuminance) / (fLum + 0.001f);
...
PS_OUTPUT_SIMPLE PS_final(PS_INPUT_SIMPLE input)
{
    ...
    // Tone mapping
    output.color.rgb *= (MIDDLE_GRAY+hdrLuminance) / (vLum + 0.001f);
    output.color.rgb *= (1.0f + output.color.rgb/LUM_WHITE);
    output.color.rgb /= (1.0f + output.color.rgb);
    
    output.color.rgb += hdrBloom * vBloom.rgb;//0.6f 
    output.color.a = 1.0f;
...
//TECHNIQUES
/////////////////////////////////////////////////////////////
technique11 sourceToLum
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_sourceToLum() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
}

technique11 downscaleLum
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_downscaleLum() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
}

technique11 brightPass
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_brightPass() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
}
...
technique11 blur
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_blur_horiz() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
	pass P1
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_blur_vert() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
}
...
technique11 adaptiveLum
{
	pass P0
	{
		SetVertexShader( CompileShader( vs_5_0, VS_identity() ) );	
		SetGeometryShader( 0 );
		SetPixelShader( CompileShader( ps_5_0, PS_calculateAdaptedLum() ) );
		SetRasterizerState(rstate_NoMSAA);    	
	}
}
